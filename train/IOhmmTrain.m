...
%        showName = {'L lane','R Lane','Straight'};
    ...
    if retrain_models
        for numiter = 1:5
            ...
            parfor i = 1:size(actions,2)
                ...
                [model,ll] = IOhmmFit(model,train_data,train_obs,model);
            ...
            save(['features/learnedModel_' train_model_for '_iohmm_NoA_' num2str(numiter) fname_ext],'learnedModels','testData','trainData','testObs',...
                'trainObs','folds_action');
            %{
            if separate_actions
                save(['observations/learnedModel_alldata_io_' num2str(numiter) fname_ext '.mat'],'learnedModels','testData','trainData','testObs','trainObs');
            else
                save(['observations/learnedModel_alldata_io_combined_' num2str(numiter) fname_ext '.mat'],'learnedModels','testData','trainData','testObs','trainObs');
            end;
            %}
        end;
    end;
    ...
    for THRESH = 0.0:0.1:1.0
        time_global = [];
       true_label = [];
    predicted_label = [];
    accuracy = []; 
        for numiter = 1:5
            load(['features/learnedModel_' train_model_for '_iohmm_NoA_' num2str(numiter) fname_ext]);
            %{
            if separate_actions
                load(['observations/learnedModel_io3_' num2str(numiter) fname_ext '.mat']);
            else
                load(['observations/learnedModel_io3_combined_' num2str(numiter) fname_ext '.mat']);
            end;
            %}
            prediction_local = [];
            true_label_local = [];
            ...
            p_lchange_local = confMat_local(1,1)*100.0/sum(confMat_local(:,1));
            p_rchange_local = confMat_local(2,2)*100.0/sum(confMat_local(:,2));
            r_lchange_local = confMat_local(1,1)*100.0/sum(confMat_local(1,:));
            r_rchange_local = confMat_local(2,2)*100.0/sum(confMat_local(2,:));
            p_iter(numiter) = mean([p_lchange_local p_rchange_local]);        
            r_iter(numiter) = mean([r_lchange_local r_rchange_local]);        
        ...
        disp(confMat./repmat(sum(confMat,2),1,size(confMat,2)));
        savename = ['Figures/confMat_iohmm_' fname_ext '_' num2str(THRESH) '.jpg'];
        %visualizeConfMat( confMat./repmat(sum(confMat,2),1,size(confMat,2)),showName,savename );
        p = [p mean([p_lchange p_rchange])];
        r = [r mean([r_lchange r_rchange])];
        p_with_std = [p_with_std std(p_iter)];
        r_with_std = [r_with_std std(r_iter)];
        p_with_mean = [p_with_mean mean(p_iter)];
        r_with_mean = [r_with_mean mean(r_iter)];
    ...
    disp(r);
    disp('Mean')
    disp(0.01*p_with_mean);
    disp(0.01*r_with_mean);
    disp('std')
    disp(0.01*p_with_std);
    disp(0.01*r_with_std);
