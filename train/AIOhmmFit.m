...
function [model,ll] = AIOhmmFit( truemodel, data, inputObs, model)
    numiter = 40;
    ll = zeros(numiter,1);
    for i = 1:numiter
        evidence = aiocalculateEvidence(model, data, inputObs);
    ...
    elseif strcmp(model.type ,'gauss')
        for i = 1:model.nstates
            ...
            for x = 1:N
                ...
                model.b = model.bparam{i};
                lgamma = gam{x};
                data_sample = data_original{x};
                T = size(lgamma,2);
                mult_const = (1.0 + sum(repmat(model.a,1,T).*inputObs{x},1)+ [0 , sum(repmat(model.b,1,T-1).*data_sample(:,1:T-1),1)])';
                
                gamma_times_data = repmat(lgamma(i,:),model.observationDimension,1).*data_original{x};
                xmu = xmu + gamma_times_data*(repmat(mult_const,1,model.observationDimension).*repmat(mu',T,1));
            end;
            ...
            model.aparam{i} = (expected.cov_matrix_input(:,:,i)) \ (expected.cov_matrix_cross(:,:,i)*sig_inv_mu*(1.0/mu_sig_inv_mu) - expected.mean_vector_input(:,i) - expected.cov_matrix_cross_lag_2(:,:,i)*model.bparam{i});


            model.bparam{i} = (expected.cov_matrix_self_lag(:,:,i)) \ (expected.cov_matrix_lag(:,:,i)*sig_inv_mu*(1.0/mu_sig_inv_mu) - expected.mean_vector_lag(:,i) - expected.cov_matrix_cross_lag(:,:,i)*model.aparam{i});
...
function expected = estep(gam,model,data,inputObs)
    ...
    for i = 1:N
        ...
        for j = 1:model.nstates
            ...
            mult_const(j,:) = (1.0 + sum(repmat(model.a,1,T).*inputObs{i},1) + [0 , sum(repmat(model.b,1,T-1).*data_sample(:,1:T-1),1)]);
        end;    
    ...
    if strcmp(model.type,'gauss')
        ...
        for i = 1:N
            ...
            for j = 1:model.nstates
                ...
                gamma_times_data_const = repmat(mult_const,model.observationDimension,1).*repmat(lgamma(j,:),model.observationDimension,1).*data_sample;
                gamma_times_data = repmat(lgamma(j,:),model.observationDimension,1).*data_sample;
                gamma_times_input_data = repmat(lgamma(j,:),model.inputDimension,1).*input_sample;

                gamma_times_data_lag = repmat(lgamma(j,2:end),model.observationDimension,1).*data_sample(:,1:end-1);
                expected.mean_vector_lag(:,j) = expected.mean_vector_lag(:,j) + sum(gamma_times_data_lag,2);
                expected.cov_matrix_cross_lag(:,:,j) = expected.cov_matrix_cross_lag(:,:,j) + gamma_times_data_lag*input_sample(:,2:end)';
                expected.cov_matrix_lag(:,:,j) = expected.cov_matrix_lag(:,:,j) + gamma_times_data_lag*data_sample(:,2:end)';
                expected.cov_matrix_self_lag(:,:,j) = expected.cov_matrix_self_lag(:,:,j) + gamma_times_data_lag*data_sample(:,1:end-1)';
                expected.cov_matrix_cross_lag_2(:,:,j) = expected.cov_matrix_cross_lag_2(:,:,j) + gamma_times_input_data(:,2:end)*data_sample(:,1:end-1)';
                ...
                expected.cov_matrix_input(:,:,j) = expected.cov_matrix_input(:,:,j) + gamma_times_input_data*input_sample';
                expected.cov_matrix_cross(:,:,j) = expected.cov_matrix_cross(:,:,j) + gamma_times_input_data*data_sample';
            end;
        end;
